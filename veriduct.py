...
def ensure_dirs(directory=DICT_DIR):
...
def get_chunk_path(hash_str):
    """Return the file path for a chunk based on its hash."""
    return os.path.join(DICT_DIR, f"{hash_str}.bin")

def chunk_exists(hash_str):
    """Check if a chunk file already exists."""
    return os.path.isfile(get_chunk_path(hash_str))

def store_chunk(hash_str, data):
    """Store the chunk if it doesn't already exist."""
    if not chunk_exists(hash_str):
        try:
            with open(get_chunk_path(hash_str), "wb") as f:
                f.write(data)
        except Exception as e:
            logging.error(f"Failed to store chunk '{hash_str}': {e}")

def file_sha256(path):
    """Calculate SHA-256 hash for a file in a memory-efficient way."""
    h = hashlib.sha256()
    try:
...
def encode_directory(input_path, out_dir, disguise=None, encrypt_key=False):
    ...
    stores them in a dictionary-based storage, and outputs a key that can be optionally
    ...
    try:
        if disguise:
            ...
            logging.info(f"Decryption key (save this securely!): {key.decode()}")
        else:
            ...
            with open(key_path, "wb") as kf:
                kf.write(compressed)
            logging.info(f"Directory encoded into '{key_path}' using dictionary: '{DICT_DIR}'")
    except Exception as e:
...
def decode_file(key_path, out_dir, disguise=None, decrypt_key=None):
    ...
    This function reads the key mapping, retrieves corresponding chunks, reconstructs
    files, and verifies the SHA-256 hash for each restored file.
    """
    try:
        if disguise:
...
def main():
    ...
    dec.add_argument("--decrypt", help="Provide decryption key if encrypted")
